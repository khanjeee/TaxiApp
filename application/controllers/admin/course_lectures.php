...
class Course_lectures extends CI_Controller {
	...
	{		
		...
		try{
			...
			$crud->set_subject('Lectures');

			$crud->required_fields('topic','topic_desc','lecture_date','day');
			$crud->columns('assign_course_id','topic','topic_desc','lecture_date','created_by','uploaded_file','uploaded_audio','section_id','batch_year');
		...
		//	$crud->callback_after_insert(array($this,'call_after_insert'));
			...
			$crud->fields('section_id','batch_year','assign_course_id','topic','topic_desc','lecture_date','created_by','created_on','uploaded_file','uploaded_audio','refer_links','tags','send_email','publish_assestment');
		...
		//$crud->display_as('assigned_by','Assignee');
			...
			$crud->display_as('topic_desc','Description');
